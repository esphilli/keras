...
@keras_export("keras.layers.DynamicLookup")
class DynamicLookup(Layer):
  ...
  and used to maintain a vocabulary. The very last index will be treated as the
  index. If `vocabulary_size=10`, OOV index will be 9.

  Args:
    vocabulary_size: Integer value representing size of the vocabulary to build.
    ...
    eviction_policy: The eviction policy for the vocabulary. Available options
      ...
      If not specified, the default eviction policy is "LFU". Expects a string.
    **kwargs: Arguments for super class.

  Attributes: get_vocabulary(): Returns a tensor representing the current
    vocabulary of the layer. If you want to look up the vocabulary keys given a
    set of indices, you can simply use `tf.gather(vocabulary, indices)`.
  ...
  def __init__(
    ...
    self.internal_table_size = tf.cast(
        tf.floor(
            tf.multiply(tf.cast(self.vocabulary_size, dtype=tf.float32), 1.2)
        ),
        dtype=tf.int32,
    )
    ...
    if self.eviction_policy == "LFU":
      ...
      raise ValueError(
          "{} eviction policy is currently unsupported by DynamicLookup layer."
  ...
  def build(self, input_shape=None):
    self.vocabulary = self.add_weight(
        ...
        dtype=self.vocabulary_dtype,
        initializer=tf.constant_initializer(self.initial_vocabulary.numpy()),
        trainable=False,
        name="vocabulary",
    )
  ...
  def call(self, inputs, learn_vocab=True):
    ...
    Args:
      ...
      learn_vocab: A boolean value that specifies whether the vocabulary should
        be learned from the layer inputs or not. Defaults to True.
  ...
  def update_internal_vocabulary(self, unique):
    ...
    new_keys = tf.sets.difference(unique_expanded, table_expanded, aminusb=True)
  ...
  def _add_new_keys(self, updated_keys, updated_values, unique, new_keys):
    ...
    if self.eviction_policy == "LFU":
      ...
      values_2 = tf.tensor_scatter_nd_add(updated_values, matches, updates)
  ...
  def get_top_vocabulary(self, k):
    ...
    if self.eviction_policy == "LFU":
      ...
      if values_len > k:
        ...
        _, indices = tf.nn.top_k(self.vocabulary_table_values, k=values_len)
