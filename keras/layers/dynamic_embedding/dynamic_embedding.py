...
from keras.utils import warmstart_embedding_matrix
import tensorflow as tf

from tensorflow.python.util.tf_export import keras_export


@keras_export("keras.layers.DynamicEmbedding")
class DynamicEmbedding(Layer):
  """A layer that updates its vocab and embedding matrix during training.

    DynamicEmbeddingLayer allows for the continuous updating of the vocabulary
    ...
    For instance, in natural language processing tasks, the vocabulary of words
    in a corpus may change over time, and it's important to update the
    ...
    this layer is used with an `UpdateEmbeddingCallback`, which is a time-based
    callback, the vocabulary lookup tensor is updated at the time interval set
    in the `UpdateEmbeddingCallback` based on the most up-to-date vocabulary
    hash table maintained by the layer. If this layer is not used in conjunction
  ...
  Args:
    ...
    initial_vocabulary: The vocabulary to initialize the layer with. If a 1D
      ...
      length `input_dim` will be generated as the initial vocabulary. Supported
      `tf.DType` values include `tf.int32`, `tf.int64` and `tf.string`.
    ...
    input_length:  Length of input sequences, when it is constant. This argument
      ...
      Expects an integer.
    embedding_initializer: Initializer for embedding vectors for new input vocab
      tokens to be added to the updated embedding matrix (see
  ...
  def update_embeddings(self, strategy):
    ...
    try:
      if isinstance(strategy, tf.distribute.ParameterServerStrategy):
        ...
        keys_list = self.dynamic_lookup_layer.vocabulary_table_keys.read_all()
        values_list = (
            self.dynamic_lookup_layer.vocabulary_table_values.read_all()
        )
        keys, values = self.aggregate_lookup_table(keys_list, values_list)
      else:
      ...
      logging.info(
          "Time interval specified by the UpdateEmbeddingCallback may be too"
